...
from harness.auth import login_required
...
def index():
    ...
    db = get_db()
    reports = db.execute(
        "SELECT p.id, created, report_value, category_id, user_two_id"
        " FROM report p JOIN user u ON p.user_id = u.id"
...
def create():
    ...
    if request.method == "POST":
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                "INSERT INTO report (user_type, body, user_id) VALUES (?, ?, ?)",
                (user_type, body, g.user["id"]),
            )
            db.commit()
